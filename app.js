...
const metricHmrDiffSpan = document.getElementById("metricHmrDiff");
const metricKwhDiffSpan = document.getElementById("metricKwhDiff");

// SAP ID Builder Elements
const sapStateCodeSelect = document.getElementById("sapStateCode");
const sapVillageCodeSelect = document.getElementById("sapVillageCode");
const sapSiteTypeSelect = document.getElementById("sapSiteType");
const sapSiteNumInput = document.getElementById("sapSiteNum");
const sapIdPreviewSpan = document.getElementById("sapIdPreview");
...
const presTopupQtyInput = document.getElementById("presTopupQty");
const presTopupDateInput = document.getElementById("presTopupDate");
...
function handleSplashScreen() {
  ...
  // Fade out splash overlay after loading progress completes (approx 300ms)
  setTimeout(() => {
...
// 6.5 SAP ID Builder Preview Updater
function updateSapIdPreview() {
  if (!sapStateCodeSelect || !sapVillageCodeSelect || !sapSiteTypeSelect || !sapSiteNumInput || !sapIdPreviewSpan) return "";
  const state = sapStateCodeSelect.value;
  const village = sapVillageCodeSelect.value;
  const type = sapSiteTypeSelect.value;
  let num = parseInt(sapSiteNumInput.value);
  if (isNaN(num) || num < 1) {
    num = 1;
  }
  const sapId = `I-${state}-${village}-${type}-${num}`.toUpperCase();
  sapIdPreviewSpan.textContent = sapId;
  return sapId;
}
...
function init() {
  try {
    ...
    // Dismiss splash screen so the app doesn't stay stuck
    const splash = document.getElementById("splashScreen");
    if (splash) splash.remove();
    
    // Alert the user with the details
    alert("Application startup error: " + error.message + "\nStack: " + error.stack);
  }
}
...
function updateResultsUI(data) {
  ...
  if (metricHmrDiffSpan) metricHmrDiffSpan.textContent = data.deltaHMR.toFixed(1);
  if (metricKwhDiffSpan) metricKwhDiffSpan.textContent = data.deltaKWH.toFixed(0);
...
function setupEventListeners() {
  ...
  // Bind change/input event listeners to SAP ID builder inputs to update live preview
  const sapInputs = [
    sapStateCodeSelect,
    sapVillageCodeSelect,
    sapSiteTypeSelect,
    sapSiteNumInput
  ];
  ...
  sapInputs.forEach(input => {
    if (input) {
      input.addEventListener("input", updateSapIdPreview);
      input.addEventListener("change", updateSapIdPreview);
    }
  });
  ...
  if (siteSubmitBtn) {
    siteSubmitBtn.addEventListener("click", () => {
      ...
      let num = parseInt(sapSiteNumInput.value);
      if (isNaN(num) || num < 1) {
        alert("Please enter a valid positive Site Number.");
        return;
      }
      const sapId = `I-${state}-${village}-${type}-${num}`.toUpperCase();
      switchActiveSite(sapId);
      alert(`Operational details for Site "${sapId}" successfully loaded!`);
    });
  }
}
...
function switchActiveSite(siteId) {
  ...
  // Parse SAP ID to populate dropdowns/inputs
  const parts = activeSiteId.split("-");
  if (parts.length === 5 && parts[0] === "I") {
    ...
    if (sapStateCodeSelect && Array.from(sapStateCodeSelect.options).some(o => o.value === state)) {
      sapStateCodeSelect.value = state;
    }
    if (sapVillageCodeSelect && Array.from(sapVillageCodeSelect.options).some(o => o.value === village)) {
      sapVillageCodeSelect.value = village;
    }
    if (sapSiteTypeSelect && Array.from(sapSiteTypeSelect.options).some(o => o.value === type)) {
      sapSiteTypeSelect.value = type;
    }
    if (sapSiteNumInput && !isNaN(num) && num >= 1) {
      sapSiteNumInput.value = num;
    }
  }
  updateSapIdPreview();
  ...
  // Load site-specific refueling history to prepopulate previous filling details
  let refuelingHistory = [];
  const refLedgerKey = `powerflow_refueling_history_${activeSiteId}`;
  const cachedHistory = localStorage.getItem(refLedgerKey);
  if (cachedHistory) {
    try {
  ...
  if (refuelingHistory.length > 0) {
    const latestRefuel = refuelingHistory[0];
    lastFillDateInput.value = latestRefuel.date || "";
    lastFillQtyInput.value = latestRefuel.qty || "";
    afterFillLevelInput.value = latestRefuel.afterFillLevel || "";
    if (document.getElementById("presTopupQty")) document.getElementById("presTopupQty").value = "";
    if (document.getElementById("presTopupDate")) document.getElementById("presTopupDate").value = "";
  } else {
    // Fallback to PM / cache defaults
    loadFuelPmInputsFromCache();
  }

  // If there's calculation history, restore the latest one and stage telemetry for the next run
  if (calculationHistory.length > 0) {
    ...
    // Previous reading for new calculation is the present reading of the last calculation
    prevHMRInput.value = latest.presHMR;
    presHMRInput.value = "";
    prevKWHInput.value = latest.presKWH;
    presKWHInput.value = "";
    ...
    if (metricHmrDiffSpan) metricHmrDiffSpan.textContent = "-";
    if (metricKwhDiffSpan) metricKwhDiffSpan.textContent = "-";
...
function applyLockUiState() {
  ...
  // Check if refueling history exists for the active site in our database ledger
  let hasRefuelHistory = false;
  const refLedgerKey = `powerflow_refueling_history_${activeSiteId}`;
  const cachedHistory = localStorage.getItem(refLedgerKey);
  if (cachedHistory) {
    try {
      const historyList = JSON.parse(cachedHistory);
      if (Array.isArray(historyList) && historyList.length > 0) {
        hasRefuelHistory = true;
      }
    } catch (e) {
      hasRefuelHistory = false;
    }
  }
  ...
  // Yield Telemetry: Lock previous values if calculation history exists, keep present values enabled
  yieldInputs.forEach(inp => {
    if (inp) {
      if ((inp === prevHMRInput || inp === prevKWHInput) && calculationHistory.length > 0) {
        inp.disabled = true;
      } else {
        inp.disabled = false;
      }
  ...
  if (activeRole === "il") {
    // IL can edit everything always, but previous baseline is read-only if refueling history exists
    fuelInputs.forEach(inp => {
      if (inp) {
        if ((inp === lastFillDateInput || inp === lastFillQtyInput || inp === afterFillLevelInput) && hasRefuelHistory) {
          inp.disabled = true;
        } else {
          inp.disabled = false;
        }
...
function handleSaveFuel() {
  ...
  const newRefuelingEntry = {
    ...
    presTopupQty: parseFloat(presTopupQtyInput.value) || 0,
    presTopupDate: presTopupDateInput.value || "",
    savedBy: activeUser,
    timestamp: new Date().toLocaleString(),
  ...
  presTopupQtyInput.value = "";
  presTopupDateInput.value = "";
...
function handleDownloadFuelReport() {
  ...
  let csv = "data:text/csv;charset=utf-8,";
  csv += "Site ID,Refueling Date,Volume Filled (Litres),Fuel Level After Filling (Litres),Topup Fuel Qty (Litres),Topup Fuel Date,Logged By,System Timestamp,Financial Year\r\n";
...
function runSelfVerificationTests() {
  ...
  try {
    ...
    // Test Case 7: SAP ID Parsing and Preview Verification
    if (sapStateCodeSelect && sapVillageCodeSelect && sapSiteTypeSelect && sapSiteNumInput) {
      const origState = sapStateCodeSelect.value;
      const origVillage = sapVillageCodeSelect.value;
      const origType = sapSiteTypeSelect.value;
      const origNum = sapSiteNumInput.value;

      sapStateCodeSelect.value = "TS";
      sapVillageCodeSelect.value = "PLVD";
      sapSiteTypeSelect.value = "ESC";
      sapSiteNumInput.value = "42";
      const tc7_preview = updateSapIdPreview();
      assertStr(tc7_preview, "I-TS-PLVD-ESC-42", "TC7: SAP ID Preview Construction (I-TS-PLVD-ESC-42 expected)");

      // Restore original values
      sapStateCodeSelect.value = origState;
      sapVillageCodeSelect.value = origVillage;
      sapSiteTypeSelect.value = origType;
      sapSiteNumInput.value = origNum;
      updateSapIdPreview();
    }

    // Test Case 8: Baseline Telemetry Input Lock Check
    if (prevHMRInput && prevKWHInput) {
      const origRole = sessionStorage.getItem("powerflow_active_role");
      sessionStorage.setItem("powerflow_active_role", "il");

      const origHistory = [...calculationHistory];
      
      // Case A: With History
      calculationHistory = [{ presHMR: 100, presKWH: 5000, make: "MM", rating: "10" }];
      applyLockUiState();
      assertBool(prevHMRInput.disabled, true, "TC8: Previous HMR is locked when calculation history exists");
      assertBool(prevKWHInput.disabled, true, "TC8: Previous kWh is locked when calculation history exists");

      // Case B: Without History
      calculationHistory = [];
      applyLockUiState();
      assertBool(prevHMRInput.disabled, false, "TC8: Previous HMR is editable when calculation history is empty");
      assertBool(prevKWHInput.disabled, false, "TC8: Previous kWh is editable when calculation history is empty");

      // Restore original state
      calculationHistory = origHistory;
      if (origRole) {
        sessionStorage.setItem("powerflow_active_role", origRole);
      } else {
        sessionStorage.removeItem("powerflow_active_role");
      }
      applyLockUiState();
